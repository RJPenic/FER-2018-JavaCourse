...
public class SmartScriptLexer {
	...
	public Token nextToken() throws SmartScriptLexerException {	/* throws SmartScriptLexerException added because of tagArgumentIdentification and readText methods. */
		...
		if (currentIndex + 1 < data.length) {									/* this if questions if the token that will be read is equal to "{$". */
		...
		if (state == SmartScriptLexerState.BASIC) {								/* if the lexer is in basic state it reads following characters as */
			...
			while (data[currentIndex] == ' ') {				/* this while loop skips all blanks since blank spaces in tags are ignorable(unless they are in string) */
				currentIndex++;
			}
			...
			if (currentIndex + 1 < data.length && (data[currentIndex] == '$' && data[currentIndex + 1] == '}')) { 	/* here, lexer checks if the next token is end-of-tag symbol. */
			...
			} else if (data[currentIndex] == '=') { 								/* Now lexer checks if the next token is =-token. Lexer needs to check this case */
				...
				while ((data[currentIndex] != ' ' || isString) && data[currentIndex] != '"') {				/* this loop will read characters until we encounter a blank or a "(that means we encountered a string). */
